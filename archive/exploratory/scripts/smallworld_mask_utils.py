...
DEFAULT_SMALLWORLD_MASK_PATH = Path("results/smallworld_mask_selfloop.pt")
...
class SmallWorldMaskSummary:
    n_nodes: int
    n_edges: int
    density: float
    original_self_loops: int
    smallworld_self_loops: int
    duplicate_edges: int
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {
            ...
            "smallworld_self_loops": self.smallworld_self_loops,
...
@register_connectome
class SmallWorldMaskConnectome:
    """Synthetic connectome with small-world-style edge incidence."""
...
def init_smallworld_mask_network(
    mask_path: Path | str = DEFAULT_SMALLWORLD_MASK_PATH,
    checkpoint_path: Path | str | None = BASELINE_CHECKPOINT,
) -> Network:
    config = _base_network_config()
    config.network.connectome.type = "SmallWorldMaskConnectome"
...
def summarize_smallworld_mask(mask_path: Path | str) -> SmallWorldMaskSummary:
    payload = load_random_mask_payload(mask_path)
    network = init_smallworld_mask_network(
        mask_path=mask_path, checkpoint_path=BASELINE_CHECKPOINT
    )
    stats = payload["stats"]
    return SmallWorldMaskSummary(
        ...
        smallworld_self_loops=int(stats["smallworld_self_loops"]),
